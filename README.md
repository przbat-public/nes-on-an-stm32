...
compared pixel by pixel (`0` means the board's framebuffer is identical to
the reference build).

| Cartridge | Mapper | On the board |
|---|---|---|
| Ignizol Combusts Literally Everything (homebrew, GPL-3.0) | NROM | plays; screenshot above; frame identical to the PC |
| Contra 2 / Super C | MMC3 | plays; AREA 1 screen identical to the PC; uses no MMC3 IRQ |
| The Lion King 3 — Timon & Pumbaa | MMC3 | plays; splits its status bar with the MMC3 IRQ, which fires on the scanline the game asks for |
| Mario 16 | MMC3 | plays; title screen identical to the PC |
| Prince of Persia (PL) | UxROM | plays; 8 KB CHR **RAM**; title screen identical to the PC |
