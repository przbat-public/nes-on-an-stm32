...
- the **mappers** that switch banks while the game runs: NROM and MMC1
  (the chip inside a large part of the NES library).
...
- [docs/BRINGUP.md](docs/BRINGUP.md) — the war stories: twelve bugs, from
  a DMA channel that could not double-buffer to a sprite-0 flag that hung
  a game a second after it started.
...
- [x] PPU: background, sprites, scrolling, NMI, sprite-0 hit, mid-frame
      scroll changes
- [x] NROM and MMC1 cartridges (MMC1: PRG/CHR banking, all four nametable
      arrangements), embedded in flash
...
third-party code and no ROM images: the test cartridge is generated from
the Python sources in `tools/`, and a real cartridge is embedded locally
