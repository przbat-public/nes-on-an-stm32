...
W etapie 11 ruszałeś duszkiem, ale świat pod nim stał w miejscu: każdy obraz, jaki dotąd
...
Konsola rozwiązuje to jednym rejestrem: niczego nie przesuwa, tylko mówi, od którego miejsca
świata zaczyna się widok. Wartość o jeden większa przesuwa cały obraz o piksel. Dzięki temu
jeden ekran pokazuje dwie rzeczy naraz: pasek statusu, który stoi, i planszę, która jedzie
pod nim.
...
wysokość świata w pikselach: 56 kafli po 8 pikseli, czyli 448. Znak `%` to dzielenie z resztą:
wynikiem jest to, co zostaje po podzieleniu, więc `(scroll_x + px) % SCREEN_W` nigdy nie
wypada poza ekran.
...
jednego kafla. Reszta z dzielenia sprawia, że świat się zawija: gdy okno dojdzie do
...
Ekran powstaje wiersz po wierszu, od góry do dołu, a rejestr jest jeden. Kiedy zmienisz go
w połowie obrazu, to, co już narysowane, zostaje w starym układzie, a reszta powstaje
w nowym. Moment, w którym wolno go zmienić, nazywa się **horizontal blank**: to krótka
przerwa między dwoma wierszami obrazu, kiedy układ obrazu nie rysuje. Zapis w tej przerwie
obowiązuje od następnego wiersza. Pilnuje tego funkcja `ppu_end_row`: po narysowaniu wiersza
kopiuje rejestry do tego, czym będzie rysował następny. Wartości zapisane w trakcie wiersza
czekają więc do przerwy, zamiast działać natychmiast.
...
Znacznik stoi tak, że jego górny wiersz wypada w ostatnim wierszu paska statusu, a w tym
wierszu leżą dwa kafle cegły, zasłonięte paskiem. Bez nich znacznik stałby nad samym niebem
i bit nigdy by się nie zapalił. Dzięki nim bit zapala się dokładnie wtedy, gdy układ obrazu
kończy pasek, i wtedy program pisze rejestr przewijania.
...
Układ obrazu to struktura `ppu_t` (PPU to układ obrazu z etapu 08): rejestry przewijania,
...
z programem nic wspólnego. Z katalogu `tutorial-pl` zbuduje go i uruchomi `make STAGE=12 run`;
plik obrazu powstanie wtedy w katalogu, z którego uruchomisz komendę.
...
kompilatorem systemowym, z biblioteką `zlib`. Z katalogu `tutorial-pl` uruchom więc:

```bash
make STAGE=12 check    # kompilacja, zero ostrzeżeń
```

`check` nic nie wgrywa i nic nie zapisuje. To samo robi ręka, z katalogu `etap-12`:
...
przy którym świat jeszcze się nie zawija, i program sprowadza ją do zera. Obraz przeskakuje.
To ten moment, w którym gra musi wczytać nowy fragment świata.

Przesuń znacznik o osiem pikseli w górę, zmieniając `SPRITE_Y` z `SPLIT_ROW - 1` na
`SPLIT_ROW - 9`. Bit zapali się wcześniej, już w dziewiątym wierszu, więc program zapisze
rejestr, zanim układ obrazu skończy pasek. Dolna część paska zniknie pod planszą, tak jak
w pliku `etap12-niecierpliwy.png`, tylko z innej przyczyny. W drugą stronę, na `SPLIT_ROW + 7`,
...
wierszy ekranu minus 16 wierszy paska. Okno dojedzie wtedy do końca świata i od dołu pokaże
...
a w pętli głównej podaj `frames`. Liczbę zamieniasz na cyfry funkcją `format_number`, tak jak
liczbę punktów: obraz to piksele, nie tekst, więc nie ma gdzie wpisać liczby.
