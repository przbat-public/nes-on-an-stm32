...
i licznik wierszy obrazu, który przerywa procesorowi pracę w połowie klatki.
...
wgrania dostajesz plik z obrazem i wydruk sprawdzeń. Kod leży w czterech plikach: `mmc5.c` to
sam układ, `ppu.c` to układ obrazu z etapu 12, przerobiony tak, żeby rysował wiersz po wierszu
i pytał kartridż o każdy kafel, `cpu.c` to procesor z etapu 13 z dołożoną obsługą przerwania,
a `main.c` składa z nich maszynę, grę i sprawdzenia. Z katalogu `etap-14` buduje się to tak:
...
buduje go, uruchamia i sprawdza wynik. Z katalogu `tutorial-pl` to samo robi `make STAGE=14 check`:
wspólny `Makefile` widzi, że etap ma własny, i oddaje mu robotę.

## Kartridż, który odpowiada na pytania

W etapie 12 układ obrazu rysował jeden wiersz po drugim. Tutaj ta sama pętla zadaje przy każdym
...
inaczej dla lewej i prawej części tego samego wiersza, a zwykły kartridż tego nie potrafi: oddaje
pamięć i milknie.

## Ekran dzielony na dwie części

Układ ma własne rejestry pod adresami od `$5100` do `$5204`: to nie pamięć, tylko okienka do
układu, tak samo jak `$2000–$2007` u układu obrazu. Rejestr `$5200` włącza podział i mówi, po
której stronie ekranu i od którego kafla się zaczyna. `$5201` to pionowa pozycja tej części
obrazu, a `$5202` jej własny bank wzorów. W tej grze prawa strona, od kafla 26, czyli od piksela
208, pokazuje drugą nametable, czyli drugą tablicę kafli, taką samą jak w etapie 08. Siedzi
w niej pasek z wynikiem; w kodzie i w wydruku sprawdzeń ten pasek nazywa się `panel`.
...
Sprawdzenie w programie rusza samym `$5201` i patrzy, czy pasek się przesunął, a świat przesunął
się tylko o swój zwykły piksel.

## Paleta dla każdego kafla

Układ ma kilobajt własnej pamięci, po jednym bajcie na kafel tła. Dwa najstarsze bity to numer
palety, a pozostałe sześć to numer banku wzorów. Włącza to zapis do `$5104`. Wiersz z pochodnią
niesie wtedy dwie palety naraz: szary kamień i ogień. Nocne niebo w oknach i napisy paska to
dwie następne, każda w swoim miejscu obrazu.
...
## Licznik wierszy i przerwanie

`$5203` to liczba wierszy, po której układ ma pociągnąć przewód przerwania, a `$5204` pozwala
temu przerwaniu przejść. Licznik liczy wiersze w trakcie rysowania. Kiedy dojdzie do celu, układ
podnosi flagę, a procesor skacze pod adres obsługi, który kartridż trzyma dla tego sygnału.

Obsługa przerwania robi dwie rzeczy. Najpierw czyta `$5204`, bo tylko odczyt kasuje flagę. Potem
wpisuje inną barwę do palety. Trafia w sam środek klatki, więc wszystko poniżej wiersza 200 jest
...
kartridża. Działa tu ten sam mechanizm okien bankowych co w etapie 13: rejestr `$5117` mówi,
który bank kartridża pokazuje się pod adresem `$A000`, a paleta leży w tym banku na początku,
więc jej szósty bajt wypada pod `$A005`.
...
kafli, mapa, paleta i poziom czekają gotowe, zanim procesor wystartuje. Rozdział zajmuje się
układem, a nie wczytywaniem grafiki.
...
`MMC5` i `SPLIT ON`. Dolnych czterdzieści wierszy ma ciepły odcień, bo tam dotarła zmiana palety.
...
**Przesuń granicę światła.** W wierszu `[0x0023] = 0xA9, 0xC8,` drugi bajt to liczba wierszy,
którą program wpisuje do `$5203` (200). Wpisz tam `0x78`, czyli 120: ciepła część obrazu
podniesie się o 80 wierszy, a dwa sprawdzenia przestaną przechodzić. Jedno pilnuje liczby 200,
a drugie sprawdza kolor muru nad tym wierszem.
...
W etapie 15 zmierzymy, gdzie w emulatorze znika czas. Najpierw pomiar na małej grze, potem dwie
albo trzy zmiany wybrane na jego podstawie. Zobaczysz tabelę, która mówi, skąd wzięła się
różnica, i klatkę wyraźnie szybszą.
